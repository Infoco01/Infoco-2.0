...
export const MailProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const getValidAccessToken = useCallback(async (): Promise<string | null> => {
        ...
        if (Date.now() >= tokens.expires_at) {
            ...
            try {
                ...
                if (!response.ok) throw new Error(data.error || "Falha ao renovar o token.");
                ...
                setError("Sua sessão com o Zoho expirou. Por favor, conecte-se novamente.");
                disconnect();
                return null;
            }
        }
    ...
    const connect = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error('Falha ao obter a URL de autenticação.');
            const data = await response.json();
